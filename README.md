...
Для того, чтобы воспользоваться скриптом, установите Perl не ниже версии 5.18, модули `Tkx` (для графического интерфейса; также вам потребуется интерпретатор Tcl v8.5+ c Tk и X11 (в Linux)), `Getopt::Long` (для интерфейса командной строки) и `Encode::Locale`, а также скачайте этот проект. Если вы планируете использовать графический интерфейс в Linux, пропишите переменную окружения `$XDG_CONFIG_DIR` или создайте каталог `~/.config` для сохранения файла конфигурации.
...
Напротив надписи «Рус. лок.» укажите каталог /localisation/ с Full-локализацией CK2 c файлами в кодировке CP1252/CP1251, обрывами строк CRLF/LF. Напротив надписи «Анг. лок.» укажите каталог /localisation/ из игры. Напротив надписи «Сохранить в» укажите каталог, в который будет сохранён результат. Выбор файлов производится по расширению `*.csv`.
...
Выберите каталоги как сказано выше. Формат входных файлов: кодировка ASCII, обрыв строки CRLF (в стиле dos). Формат выходных файлов такой же. Выбор файлов производится по расширениям `*.fnt`, `*.tga` и `*.dds`.
...
Выберите каталоги как сказано выше. Для перекодировки нажмите на кнопку с желаемым действием. Выбор файлов производится по расширению `*.yml`.
...
Выберите каталоги как сказано выше. Для перекодирования нажмите на кнопку. Производится выбор всех файлов (не каталогов).
